...
// Определяем enum для статусов аукционов
export const auctionStatusEnum = {
  ACTIVE: "active", // Активный аукцион
  ARCHIVED: "archived", // Архивный аукцион
};
...
export const auctions = pgTable(
  "auctions",
  {
    ...
    status: text("status", { enum: Object.values(auctionStatusEnum) })
      .notNull()
  ...
  (auction) => ({
    // Index for sorting by creation date (most important for listing auctions)
    createdAtIdx: index("auctions_created_at_idx").on(auction.createdAt),

    // Index for filtering by status
    statusIdx: index("auctions_status_idx").on(auction.status),

    // Composite index for status and creation date (for filtered sorting)
    statusCreatedAtIdx: index("auctions_status_created_at_idx").on(
      auction.status,
      auction.createdAt,
    ),

    // Index for user's auctions
    userIdIdx: index("auctions_user_id_idx").on(auction.userId),
  }),
);
